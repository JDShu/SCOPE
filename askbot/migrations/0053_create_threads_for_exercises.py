...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print "Converting exercise to threads:"
        num_exercises = orm.Exercise.objects.count()
        for exercise in ProgressBar(orm.Exercise.objects.iterator(), num_exercises):
            thread = orm.Thread.objects.create(favourite_count=exercise.favourite_count)
            exercise.thread = thread
            exercise.save()

        if orm.Exercise.objects.filter(thread__isnull=True).exists():
            raise ValueError('There are Thread-less exercises after this migration has been applied!')

        if orm.Exercise.objects.exclude(favourite_count=models.F('thread__favourite_count')).exists():
            raise ValueError("There are Thread instances for which data doesn't match Exercise!")
    ...
    models = {
        ...
        'askbot.exercise': {
            ...
            'text': ('django.db.models.fields.TextField', [], {'null': 'True'}),
            'thread': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'exercises'", 'unique': 'True', 'null': 'True', 'to': "orm['askbot.Thread']"}),
            'title': ('django.db.models.fields.CharField', [], {'max_length': '300'}),
