...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for exercise in ProgressBar(orm.Exercise.objects.iterator(), num_exercises, message):
            ...
            thread.closed = exercise.closed
            thread.closed_by = exercise.closed_by
            thread.closed_at = exercise.closed_at
            thread.close_reason = exercise.close_reason

            thread.save()

        if orm.Exercise.objects.exclude(closed=models.F('thread__closed')).exists():
        ...
        for field_name in ['closed_by', 'closed_at', 'close_reason']:
            ...
            if orm.Exercise.objects.filter(closed=True).exclude(**kwargs).exists():
                raise ValueError("There are Thread instances for which data doesn't match Exercise!")
