...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        message = "Marking accepted problem in threads"
        num_threads = orm.Thread.objects.count()
        for thread in ProgressBar(orm.Thread.objects.iterator(), num_threads, message):
            if thread.accepted_problem:
                thread.accepted_problem_post = orm.Post.objects.get(self_problem__id=thread.accepted_problem.id)
                thread.save()

        message = "Connecting exercise view objects to posts"
        num_exercise_views = orm.ExerciseView.objects.count()
        for qv in ProgressBar(orm.ExerciseView.objects.iterator(), num_exercise_views, message):
            qv.exercise_post = orm.Post.objects.get(self_exercise__id=qv.exercise.id)
            qv.save()

        message = "Connecting anonymous problems to posts"
        num_anon_problems = orm.AnonymousProblem.objects.count()
        for aa in ProgressBar(orm.AnonymousProblem.objects.iterator(), num_anon_problems, message):
            aa.exercise_post = orm.Post.objects.get(self_exercise__id=aa.exercise.id)
            aa.save()
        ...
        for rev in ProgressBar(orm.PostRevision.objects.iterator(), num_post_revs, message):
            if rev.exercise:
                assert not rev.problem
                rev.post = orm.Post.objects.get(self_exercise__id=rev.exercise.id)
            elif rev.problem:
                assert not rev.exercise
                rev.post = orm.Post.objects.get(self_problem__id=rev.problem.id)
            else:
                raise ValueError('PostRevision with neither exercise nor problem found!')
            rev.save()
