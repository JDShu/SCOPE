...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        message = "Adding accepted problems to threads"
        num_exercises = orm.Exercise.objects.count()
        for exercise in ProgressBar(orm.Exercise.objects.iterator(), num_exercises, message):
            thread = exercise.thread

            if exercise.problem_accepted:
                try:
                    accepted_problem = exercise.problems.get(accepted=True)
                except orm.Problem.DoesNotExist:
                    # Unfortunately there's probably nothing we can do in this case,
                    # there's no record of which problem has been accepted (HMM, MAYBE IN ACTIVITIES???)
                    print TERM_YELLOW, "Found a thread with exercise.problem_accepted, " \
                        "but no problem actually marked as accepted, exercise.id=%d." \
                        "This happens so there's no need to panic." % exercise.id, TERM_RESET
                else:
                    thread.accepted_problem = accepted_problem
                    thread.problem_accepted_at = accepted_problem.accepted_at
                    thread.save()

        # Verify data integrity
        message = "Checking correctness of accepted problem data"
        for exercise in ProgressBar(orm.Exercise.objects.iterator(), num_exercises, message):
            accepted_problems = exercise.problems.filter(accepted=True)
            num_accepted_problems = len(accepted_problems)

            if( (num_accepted_problems == 0 and exercise.thread.accepted_problem) or
                (num_accepted_problems == 1 and not exercise.thread.accepted_problem) or
                (num_accepted_problems == 1 and exercise.thread.accepted_problem != accepted_problems[0]) or
                (num_accepted_problems == 1 and exercise.thread.problem_accepted_at != accepted_problems[0].accepted_at) or
                (num_accepted_problems > 1) ):
                raise ValueError("There are Thread instances for which data doesn't match Exercise/Problems!")
