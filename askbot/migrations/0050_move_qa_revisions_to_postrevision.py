...
class Migration(DataMigration):
    ...
    def copy_revision(self, orm, source_revision):
        exercise = getattr(source_revision, 'exercise', None)
        problem = getattr(source_revision, 'problem', None)

        if bool(exercise) == bool(problem): # one and only one has to be set (!xor)
            raise ValueError('Strange source revision! Check this manually')

        if (exercise and source_revision.revision_type != 1) or (problem and source_revision.revision_type != 2):
        ...
        # - it's because `revision edited` Activities point to Exercise&Problem instances
        # ans not to their relevant revisions. (BTW this might be considered a bug)


    def forwards(self, orm):
        # Process revisions
        for qr in orm.ExerciseRevision.objects.iterator():
            self.copy_revision(orm=orm, source_revision=qr)

        for ar in orm.ProblemRevision.objects.iterator():
