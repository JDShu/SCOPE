...
Q_INDEX_NAME = 'askbot_exercise_full_text_index'
A_INDEX_NAME = 'askbot_problem_full_text_index'
        ...
        if db.backend_name == 'mysql':
            if mysql.supports_full_text_search_migr0004():
                ...
                exercise_index_sql = get_create_full_text_index_sql(
                                                Q_INDEX_NAME,
                                                orm.Exercise._meta.db_table,
                                                ('title','text','tagnames',)
                                            )
                db.execute(exercise_index_sql)

                problem_index_sql = get_create_full_text_index_sql(
                                                A_INDEX_NAME,
                                                orm.Problem._meta.db_table,
                                                ('text',)
                                            )
                db.execute(problem_index_sql)
            else:
