...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for exercise in ProgressBar(orm.Exercise.objects.iterator(), num_exercises, message):
            exercise.thread.tags.add(*list(exercise.tags.iterator()))

        if orm.Exercise.objects.annotate(tag_num=models.Count('tags'), thread_tag_num=models.Count('thread__tags')).exclude(tag_num=models.F('thread_tag_num')).exists():
    ...
    models = {
        ...
        'askbot.favoriteexercise': {
            ...
            'user': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'unused_user_favorite_exercises'", 'to': "orm['auth.User']"})
        },
        'askbot.markedtag': {
