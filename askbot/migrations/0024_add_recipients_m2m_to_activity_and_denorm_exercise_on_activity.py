...
#some of activities are not related to exercise, so they are not processed here
APPROPRIATE_ACTIVITIES = (
    const.TYPE_ACTIVITY_ASK_EXERCISE,
    const.TYPE_ACTIVITY_UPDATE_EXERCISE,
    const.TYPE_ACTIVITY_DELETE_EXERCISE,
    const.TYPE_ACTIVITY_UPDATE_TAGS,
    const.TYPE_ACTIVITY_PROBLEM,
    const.TYPE_ACTIVITY_UPDATE_PROBLEM,
    const.TYPE_ACTIVITY_DELETE_PROBLEM,
    const.TYPE_ACTIVITY_MARK_PROBLEM,
    const.TYPE_ACTIVITY_COMMENT_EXERCISE,
    const.TYPE_ACTIVITY_COMMENT_PROBLEM,
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        #2) save exercise value into the activity 
        for act in orm.Activity.objects.all():
            if act.activity_type in APPROPRIATE_ACTIVITIES:
                try:
                    act.exercise = api.get_origin_post_from_content_object(act)
                    act.save()
                except Exception, e:
                    have_problems = True
