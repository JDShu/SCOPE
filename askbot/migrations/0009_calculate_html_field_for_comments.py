...
def get_authors(exercise, orm = None):
    authors = set()
    for q_rev in orm.ExerciseRevision.objects.filter(exercise=exercise):
    ...
    problems = orm.Problem.objects.filter(
                                exercise = exercise,
                                deleted = False
                            )
    for a in problems:
        for a_rev in orm.ProblemRevision.objects.filter(problem = a):
            authors.add(a_rev.author)
        a_comments = orm.Comment.objects.filter(
                                        content_type__model = 'problem',
                                        object_id = a.id
                                    )
...
def mentionize(text, comment = None, all_users = None, orm = None):
    if comment.content_type.model == 'problem':
        problem = orm.Problem.objects.get(id = comment.object_id)
        exercise = problem.exercise
    else:
        exercise = orm.Exercise.objects.get(id = comment.object_id)

    #build ordered list of author names
    authors = get_authors(exercise, orm = orm)
